...
- 대상: ./logs/signals_upbit.csv (기본)
- 전략: 롱 온리, 24h 만료, 왕복 0.1% (0.05%/0.05%)
- 엔트리: 신호 발생 '다음 봉 시가'
- 이벤트 필터: line_breakout / box_breakout + side == support (상향 돌파만 롱)
- tz 안전, 심볼별 OHLCV 캐시, 바 단위 디듀프(심볼+bar_ts)

사용:
  python backtest_upbit_multi_strategies.py
  # 또는
  python backtest_upbit_multi_strategies.py --signals ./logs/signals_upbit.csv --timeframe 15m
"""
...
def floor_to_frame(ts: pd.Timestamp, minutes: int) -> pd.Timestamp:
    ts = to_utc(ts)
    base = ts.replace(minute=0, second=0, microsecond=0)
    q = (ts.minute // minutes) * minutes
    return base + pd.Timedelta(minutes=q)
...
# ------------------------ 시그널 로드/정리 ------------------------
def load_upbit_signals(path: str, timeframe: str, dedupe_per_bar: bool = True) -> pd.DataFrame:
    df = pd.read_csv(path)
    ...
    df["ts"] = pd.to_datetime(df["ts"], utc=True, errors="coerce")
    df = df.dropna(subset=["ts","symbol","side","event"]).reset_index(drop=True)

    # 이벤트/사이드 필터: 상향 돌파만 롱
    df["event"] = df["event"].str.lower()
    df["side"]  = df["side"].str.lower()
    df = df[df["event"].isin(["line_breakout","box_breakout"])]
    df = df[df["side"] == "support"].copy()

    # 타임프레임 정합(파일에 없으면 전체를 사용)
    if "timeframe" in df.columns and df["timeframe"].notna().any():
        df["timeframe"] = df["timeframe"].fillna("").astype(str).str.lower()
        df = df[df["timeframe"] == timeframe.lower()].copy()
    else:
        df["timeframe"] = timeframe

    # 바 단위 디듀프: 한 심볼·한 바에서 최대 1개만 트레이드
    if dedupe_per_bar:
        minutes = tf_to_minutes(timeframe)
        df["bar_ts"] = df["ts"].apply(lambda x: floor_to_frame(x, minutes))
        df = df.sort_values(["symbol","bar_ts","ts"])
        df = df.drop_duplicates(subset=["symbol","bar_ts"], keep="first").reset_index(drop=True)

    return df[["ts","symbol","timeframe"]].sort_values("ts").reset_index(drop=True)
...
def main():
    ...
    ap.add_argument("--signals", default="./logs/signals_upbit.csv")
    ap.add_argument("--timeframe", default="15m")
    ap.add_argument("--out-trades", default="./logs/bt_upbit_trades.csv")
    ap.add_argument("--out-stats",  default="./logs/bt_upbit_stats.csv")
    args = ap.parse_args()

    ensure_dir(args.out_trades); ensure_dir(args.out_stats)

    df_sig = load_upbit_signals(args.signals, args.timeframe, dedupe_per_bar=True)
    ...
    print("\n=== Upbit Backtest (Long-only / 24h expiry / 0.1% fees) ===")
    if trades_df.empty:
        print("No trades produced.")
        return
    print(f"Signals used : {len(df_sig)} rows, {df_sig['symbol'].nunique()} symbols")
