...
function Load-DotEnv([string]$Path){
    if (-not (Test-Path $Path)) { Write-Warning '[WARN] .env not found: {0}' -f $Path; return }
...
if ($RunPipeline) {
    try {
        ...
        if (Test-Path $pipe) {
            ...
            Write-Verbose '[OK] Pre-pipeline executed.'
        } else {
            Write-Warning '[WARN] run_pipeline.ps1 없음 - 건너뜀'
        }
    } catch {
        Write-Warning ('[WARN] 사전 파이프라인 실패: {0}' -f $_.Exception.Message)
    }
}
...
foreach($row in $trades){
    ...
    # 패턴 A: entry=quick_expired(문자), exit=0.001(숫자), pnl 공란
...
$toList = @()
if ($To) { $toList = $To -split '\s*,\s*' | Where-Object { $_ -and $_.Trim() -ne '' } }
...
if ($attachments.Count -gt 0) { $sendParams.Attachments = $attachments }

Send-MailMessage @sendParams
Write-Host ("[OK] Mail sent to {0} ({1})" -f ($toList -join ', '), $subject) -ForegroundColor Green