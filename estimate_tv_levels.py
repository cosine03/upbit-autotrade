...
def series_to_ns_utc(s: pd.Series) -> np.ndarray:
    ...
    # pandas 2.2 호환: view 대신 .astype 사용
...
def cluster_by_price(df: pd.DataFrame, idxs: np.ndarray, band: float) -> List[Dict]:
    """가격 근접도(band)로 간단 클러스터링 (이전 답변에서 사용했던 로직)."""
    if idxs.size == 0:
        return []
...
def estimate_level_for_signal(
    ...
    반환: {est_level, est_band, est_touches, distance_pct, method}
    """
    # 1) OHLCV 로드 & ts 정규화
    ...
    # 2) 알람 시점이 포함되는 봉 찾기
    #    (봉 시작 ~ 끝 사이면 그 봉 close를 알람 당시 가격 proxy로 사용)
    ...
    if i < 0:
        return None
    # 알람 직전/해당 봉 close를 "알람 당시 가격"으로 사용
