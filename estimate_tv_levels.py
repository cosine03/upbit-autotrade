# -*- coding: utf-8 -*-
"""
estimate_tv_levels.py (Single Process, Full Patched Version)
- TradingView 알람 시점의 레벨을 근사 추정해서 signals_tv.csv를 보강
- OHLCV CSV 파일을 지정된 폴더에서 읽고, 알람 당시 레벨/거리/터치수를 추정
"""
...
def series_to_ns_utc(s: pd.Series) -> np.ndarray:
    ...
    return s.astype("int64").to_numpy()   # FutureWarning 대응
...
def normalize_swings(swings, n_rows: int) -> Dict[str, np.ndarray]:
    def to_int_idx(x):
        ...
        if n_rows: arr = np.clip(arr, 0, n_rows-1)
...
def estimate_level_for_signal(symbol: str, sig_ts: pd.Timestamp, side_hint: Optional[str],
    ...
    df = get_ohlcv(symbol, timeframe, roots=roots, patterns=patterns, assume_tz=assume_tz)
    ...
    cands = []
    if side_hint == "support": cands += [("support", lv) for lv in sup_levels]
    elif side_hint == "resistance": cands += [("resistance", lv) for lv in res_levels]
    else: cands += [("support", lv) for lv in sup_levels] + [("resistance", lv) for lv in res_levels]
...
def main():
    ...
    if "symbol" not in df.columns:
        ...
        else: raise ValueError("signals 파일에 'symbol' 컬럼 필요")

    if args.limit > 0: df = df.iloc[:args.limit].copy()

    roots = args.ohlcv_roots.split(";")
    patterns = args.ohlcv_patterns.split(";")
    ...
    for k, row in df.iterrows():
        est = estimate_level_for_signal(
            symbol=row["symbol"], sig_ts=row["ts"], side_hint=parse_side(row),
            timeframe=str(row.get("timeframe", args.timeframe)),
            lookback=args.lookback, lookahead=args.lookahead,
            roots=roots, patterns=patterns, assume_tz=args.assume_ohlcv_tz
        )
        out = dict(row)
        if est: out.update(est)
        else:
            out.update({"est_level": np.nan, "est_band": np.nan,
                        "est_touches": np.nan, "distance_pct": np.nan,
                        "method": "failed", "side_used": "", "sig_price": np.nan})
        rows.append(out)

        if (k+1) % 50 == 0: print(f"[EST] processed {k+1}/{total} ...")
